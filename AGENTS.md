...
main.js Entry point: boot (restore the saved place, migrate old prefs).
...
settings.js The ☰ Settings menu: default translation, appearance
(System/Light/Dark), tradition (the canon).
...
- State lives in a single `state` object; per-viewer prefs persist to
  `localStorage` under the key `verbum-prefs`: book, chapter, `translation` (the **default**
  translation from Settings), canon, Greek-line toggle, and theme (`''` = follow the system). The
  top-bar translation menu is a switch for the current visit: it's kept in `sessionStorage`
  (`verbum-session`), so it survives a reload of the tab, but a new visit opens in the default. All
  storage access is wrapped in try/catch, since it can throw or be unavailable. The saved theme is
  also applied by a tiny inline script in `index.html`'s head, so there's no theme flash before the
  modules run.
...
- The canon (`state.canon`: `protestant` (default), `catholic`, `orthodox`; the profiles in
  `catalog.json`; chosen as "Tradition" in Settings) applies in four places: the book list, the reader (a verse is hidden only when all
