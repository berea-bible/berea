...
data.js The app's data layer: the berea-data instance (`lib`),
catalog helpers (book names, canon profiles, book
lists), and the live NASB/ESV proxy code.
...
data/ Pre-v2 compatibility output (nothing reads it any more;
removed in phase 6 of docs/v2-plan.md).
...
dist/ The data the app reads, keyed by the shared verse ID
(catalog.json, text/, orig/, lex/, comm/, validation.json,
versification.txt). Generated by the build.
...
- api.bible terms §14 (FUMS) is mandatory for webapps: every display of NASB
  text must call `reportNasbView(book, chapter)` (`js/data.js`, book = code), which sends
